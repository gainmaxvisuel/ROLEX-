  ...
  try {
    ...
    */
    ...
    showToast(
      "Erreur",
      error.message ||
      "Impossible de charger la page de dépôt.",
      "error"
    );
...
function renderPaymentMethods() {
  ...
  receiverBox.classList.remove("show");
  ussdBox.classList.remove("show");
  paymentButton.classList.remove("show");
  beninTogoInfo.classList.remove("show");
  ...
  */
  ...
  if (
    ...
    if (!method) {
      ...
      throw new Error(
        "Aucun numéro de réception n'est configuré pour votre pays."
      );
...
function updateUSSD() {
  ...
  /*
    Pas d'USSD pour Wave,
    Moov CI, MTN CI, Orange CI.
  */
...
paymentButton.addEventListener(
  ...
  () => {
    ...
    /*
      ...
      Aucun OTP n'est demandé ou stocké.
    */
...
depositForm.addEventListener(
  ...
  async event => {
    ...
    /*
      Limite correspondant au bucket
      payment-proofs : 10 Mo.
    */
    ...
    try {
      ...
      const fileName =
        `${Date.now()}-${randomString(10)}.${safeExtension}`;
      ...
      if (uploadError) {
        ...
        throw new Error(
          "Le dépôt a été créé, mais la preuve de paiement n'a pas pu être envoyée. Veuillez contacter le support."
        );
